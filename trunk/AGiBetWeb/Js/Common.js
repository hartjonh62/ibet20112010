...
function doBlinḳAccInfoTab() {    
    if ($('lblAccInfo')) {
        if ($('lblAccInfo').style.color=='white') 
        {            
            $('lblAccInfo').style.color='red'; 
        }
        else                 
        {
            $('lblAccInfo').style.color='white';
        }
    }
}
...
{
    clearInterval(blinkAccInfo)
    $('lblAccInfo').style.color='';
    $('lnkAccInfo').className = "tab_menu02";
}
...
function slide_down() {
    ...
    roleID = getRoleID();
    if (roleID == 4) {    
        blinkAccInfo = setInterval("doBlinḳAccInfoTab()", 450);
    }    
}
...
function OpenUserGuide() {        
    var _url ='wap_use.aspx';
    var _w =window.open(_url,'flashuserguide','height=463,width=800,menubar=no,status=no,scrollbars=no,resizable=no');
...
function RedirectHRFICS(custid, isHR, isFI, isCS) {
    if (isHR == 0 || isFI == 0 || isCS == 0) {
        url = "../MemberInfo/RedirectPT.aspx?custid=" + custid + "&ishr=" + isHR + "&isfi=" + isFI + "&iscs=" + isCS + "&isedit=false" + "&insert=true";
    }   
    else {
        DelayReloadPage("../MemberInfo/DownlineList.aspx", 2000);
        return;
    }
    DelayReloadPage(url,5000);
}
...
var arrPlaceHolder = ["err", "error1", "error2", "error3", "error5", "error8", "error10", "error11", "error201", "error99", "error151"];
var arrPanel = ["", "panelSoccer", "panelBasket", "panelFootball", "panelTennis", "panelBaseball", "panelGolf", "panelMoto", "panelFinancial", "panelOtherSports", "panelHorseracing"];
function ShowError(errMsg, placeHolder) {
    ...
    for (var i = 0; i < arrPlaceHolder.length; i++) {
        if (placeHolder == arrPlaceHolder[i]) {
            ...
            if ($(arrPlaceHolder[i]))
                $(arrPlaceHolder[i]).innerHTML = "";
            if ($('div' + arrPlaceHolder[i]))
                $('div' + arrPlaceHolder[i]).style.display = "none";
        }
    }
    if (isEdit) {
        if ($("btnSubmit"))
...
function createCheck(id) {
    ...
    t.border = 0; t.style.marginBottom = '-4px';
...
function ViewHistory(RoleID) {
    ...
    var IsHistotyReport = $('IsHistotyReport').value;
    IsHistotyReport = (IsHistotyReport == '1') ? '0' : '1';
    ...
    if (current_url.indexOf('Detail') > 0) {
        if (current_url.indexOf('DetailSuper') > 0)
            url = 'WinlossDetailSuper.aspx';
        else if (current_url.indexOf('DetailMaster') > 0)
            url = 'WinlossDetailMaster.aspx';
        else if (current_url.indexOf('DetailAgent') > 0)
            url = 'WinlossDetailAgent.aspx';
    }
    else {
        switch (RoleID) {
            case 4: url = 'WinlossSuper.aspx'; break;
            case 3: url = 'WinlossMaster.aspx'; break;
            case 2: url = 'WinlossAgent.aspx'; break;
        }
    }
    ...
    url += '&IsHistotyReport=' + IsHistotyReport;
    url += '&IsSwitch=1';
...
function DisableHR(idchkHR, ischkHRDownline) {
    ...
    if ($('hdDisabledHR_Old').value == '0' && $('hdDisabledHRDownline_Old').value == '0') {
        if ($(ischkHRDownline)) $(ischkHRDownline).checked = $(idchkHR).checked;
    }
    else if ($('hdDisabledHR_Old').value == '1' && $('hdDisabledHRDownline_Old').value == '1') {
        if ($(ischkHRDownline)) {
            $(ischkHRDownline).disabled = !$(idchkHR).checked;
            if ($(idchkHR).checked == false)
                $(ischkHRDownline).checked = false;
        }
    }
    else if ($('hdDisabledHR_Old').value == '0' && $('hdDisabledHRDownline_Old').value == '1') {
    ...
    $('hdDisabledHR').value = $(idchkHR).checked ? '0' : '1';
    if ($(ischkHRDownline)) {
        $('hdDisabledHRDownline').value = $(ischkHRDownline).checked ? '0' : '1';
    }    
}
...
function DisableFI(idchkFI, ischkFIDownline) {
    ...
    if ($('hdDisabledFI_Old').value == '0' && $('hdDisabledFIDownline_Old').value == '0') {
        ...
        if ($(ischkFIDownline)) {
            ...
            if ($(idchkFI).checked == false)
                $(ischkFIDownline).checked = false;
        }
    }
...
//nqminh - 20Feb09 - show flash introduction popup from main frame!
//Example: showFlashPopup('wap');                   -> this will show flash from Flash/wap.swf
//     or  showFlashPopup('intro', 600, 400);       -> this will show flash from Flash/intro.swf with size(600, 400)
function showFlashPopup(flashName, flashWidth, flashHeight) {
    if (typeof flashWidth == 'undefined') flashWidth = 500;
    if (typeof flashHeight == 'undefined') flashHeight = 400;
    if (top.main)
        top.main.alertPopupFromMe(flashName, flashWidth, flashHeight);
}

function alertPopupFromMe(flashName, flashWidth, flashHeight) {
    if (typeof ibcWnd != 'undefined' && ibcWnd)
        ibcWnd.open('../Ad1.aspx?fn=' + flashName + '&fw=' + flashWidth + '&fh=' + flashHeight, '', 100, 100, flashWidth, flashHeight, 'class1', '');
}
    ...
    if (top.main) {
        if (top.main.location.href.toLowerCase().indexOf('info/creditbalance.aspx') >= 0) {
            if (typeof top.main.alertDivPopup == 'function') {                
                if (!top.main.$('accinfomas') || top.main.$('accinfomas').style.display == 'none')
                    top.main.alertDivPopup(popupName);
            }
        }
    }
}
...
function addDivPopup(divContentId) {
    if (!hasCookieName(divContentId)) {        
        ...
        if (top.main.location.href.toLowerCase().indexOf('info/creditbalance.aspx') >= 0) {            
            if (top.main.$('divPopupLetter'))
                top.main.$('divPopupLetter').style.display = 'none';
        }
...
function viewMixParlayResult(refno,winlostdate) {
    ...
    ibcWnd.open('../Popup/ViewMixParlayResult.aspx' + params, '', 50, 100, width, height, 'class1', 'class2');
}
...
function viewFinanceResult(transid) {
    ...
    ibcWnd.open('../Popup/ViewFinanceResult.aspx' + params, '', 300, 100, width, height, 'class1', 'class2');
}

function EditMember_Single(cid, user, agentid, Customer)
{
	var URL = "../UserAgent/EditMember.aspx?"; //V1
	//var URL = "../UserV2/UserAgent/EditMember.aspx?"; //V2
